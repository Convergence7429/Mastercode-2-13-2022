...
public class DriveTrain {
    ...
    public void driveTrainByControls() {
        ...
        if (isDriverControlEnabled) {
            double ySpeed = Constants.xbox.getRawAxis(1);
            double xSpeed = Constants.xbox.getRawAxis(0);
            double zSpeed = Constants.xbox.getRawAxis(2);

            double yDirectionMaintainer = 1.0;
            double xDirectionMaintainer = 1.0;
            double zDirectionMaintainer = 1.0;

            if ((ySpeed > -motorSpeedThresholdTeleop) && (ySpeed < motorSpeedThresholdTeleop)) { // minimum thresholds
                                                                                                 // to go
                                                                                                 // so no interference
                ySpeed = 0.00;
            }
            ...
            mecanumDrive.driveCartesian(-ySpeed * ySpeed * yDirectionMaintainer, xSpeed * xSpeed * xDirectionMaintainer,
                    zSpeed * zSpeed * zDirectionMaintainer);
        }
    }
